...
#There are numerous "frameworks" you can use to build a 
...
#We can make this easier by creating an SDK to work with 
# the Db (as we did)
#Long term goal is to actually create an API we can consume...
#We'll get into this, but essentially we can have the 
# same backend for:
#   Desktop app
#   CLI
#   Web App
...
#For now we are going to use the SDK, 
# build out a UI, and then later an API

"""
#Getting started
#Because tkinter is built in, 
# we don't have to download or install anything.
#We just import it:
import tkinter
...
tk = tkinter.Tk()#to start app
tk.title("Guessing Game")#title of window
guess_goal = tkinter.Label(tk, text=f"Guess a number {low} to {high} (inclusive)")
guess_goal.pack()
entry = tkinter.Entry(tk)#input box
entry.pack()
button = tkinter.Button(tk, text="Take a guess",
 command=lambda: check(int(entry.get())))
button.pack()
tk.mainloop()
