...
conn = sqlite3.connect(":memory:")#to store database in memory
#conn = sqlite3.connect("database.db")#to create a database file

#creating this goes off of the cwd.  CD in terminal changes cwd
#so either cd to correct folder or figure out 
# how to do a more concrete path:
#import os.path
...
# Create table
c.execute('''CREATE TABLE books (title TEXT, pages INTEGER)''')#3 quotes for multiple lins
...
c.execute("INSERT INTO books VALUES ('Are You My Mother?', 72)")#inserting with both values
#c.execute('INSERT INTO books (title) VALUES ("Are You My Mother?")')#inserting one value
...
print(c.fetchone())#fetches a tuple, all fetches a list
#data=c.fetchone()
#print(data)
