...
#I'm not sure of exact internals on how the hash is used, 
# but imagine it like so:
#You have an area of memory with 8 spots, 
# and you need to store the value at some spot...
print(hash("hello") % 8) 

#Almost always immutable type (should be, anyway)
#a tuple, string, int will work, list will not. a number will work

#Why use a hashtable? Extremely fast to add or look up data
#O(1) --> constant time. More elements does not mean slower 
# unlike a sort or something
...
#but this is different than a list where we iterate to find 
# the element
#The key goes through hash to calculate index. 
# Either there or not
#This is O(1)

#You may still not like the casing and in that situation 
# there is a method

print(emails.get("Ryan")) #Returns none if not found
print(emails.get("Ryan", "Not found")) 
...
emails[5] = "test"
emails[(1, 2)] = "yep"
#emails[[5, 3]] = "nope" #list is not hashable 
# (mainly cuz mutable)
...
#Not ideal.
#One reason being the key has to be hashed to get the 
...
#Each iteration k will be the key and elem will be the item found
#  at this key.

#As an example of what a hashtable can be used for, 
# you can keep track of occurances:

conjunctions = {"but": 0, "or": 0, "so": 0, "and": 0, 
"yet": 0, "for": 0, "nor": 0} #fanboys

completely_original_poem = """I still hear your voice when you 
sleep next to me
...
#This could easily be wrapped in a function to take a msg 
# and words to look for, returning a dict
#concept can be used to analyze documents to quantify how vulgar 
# they are, search for phrases, etc
#dictionaries can be used to keep track of values that are hard 
# to calculate (memoization)


######### SETS EXPLAINED #########


#Sets are similar to dictionaries in that the data is 
# hashed and it is unordered.
#Sets are similar to lists in that they just 
# contain the data and not a key-value pair
#Sets are different than lists in that 
...
#Behind the scenes, they both use hashing. 
# The hashing is used to determine where to store the data.
...
#An example would be to see if a word is ever used in a phrase.
#  Not counted (that would be a dictionary)

conjunctions = {"but", "or", "so", "and", "yet", "for", "nor"} #fanboys
seen = set() #THERE'S NOT AN EMPTY SET LITERAL!! #learn something
# new every day
...
#You can remove duplicate elements from a list by 
# converting it to a set and back. 
...
#Earlier on in our life I showed some code to count each type
...
#This works because is iterates through the 
# set {"red", "green", "blue"} counting each in colors
...
print(only_my_colors) #elements in left getting rid of all in 
#right. 
#Could go other way too:
only_her_colors = her_fav - my_fav
print(only_her_colors)

#symmetric difference is like if you took colors only I liked 
# union with colors only she liked and put em together:
...
# there are method versions that return. --> 
