...
#This section is designed to be a simple introduction to object 
# oriented programming
#We are in no way covering everything... 
#Probably will do OOP or intermediate Python series dedicated 
# to this stuff
...
#This object is assigned to the book variable
...
#This is cool and all but every book has a title, 
# so we can make it part of the structure
...
#consider this the constructor, or the function that is invoked 
# when we create a book.
#self refers to the book being created and we don't have to worry 
# about passing that, it's implicit
#Title is passed in as an argument, 
# and assigned to self.title (the title of the book)
...
#If you want the title to be optional you can give it a default 
...
#We can define any functions to be attached to our objects 
# by putting em in our class
...
book.is_long() #no need to pass anything to self parameter. 

#A note on __init__
#__init__ is also a method, but it's special in that it is invoked
#  automatically
#When we create an object.
#There are a lot of methods with a similar naming structure 
# we are going to see. 
...
#Which would work but not ideal as we'd easily be overwhelemed 
# with so many functions
#And no organization. It's ideally attached to the object 
...
#Coming from another language, this would be the closest thing to 
# "static" members
...
#if we leave off self, we are creating it at the class level, 
# which is shared for all objects
...
print(book)#prints type nand address
print(book==book2)#prints False, compares area of memory to see if same object
Book.favs.append(book)
Book.favs.append(book2)

print(Book.favs) #Favs is related to books, 
#but not tied to one individual book
...
#To save save space we are combing the code for all three of these
#  sections.
...
class Book():
    ...
    #It's approriate to give something for __hash__ when you 
    # override __eq__
...
print("Are they considered equal?", book == equal_book) #yep, uses __eq__
